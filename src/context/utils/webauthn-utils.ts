...
import type { SSE_Account, SSE_User, InternalOptions } from "../types";
import type {
  Authenticator,
  ResponseInternal,
  Awaited,
...
import { webauthnChallenge } from "../actions/checks";
import type {
...
export function assertInternalOptionsWebAuthn(
  ...
  if (!adapter)
    throw new Error("An adapter is required for the WebAuthn provider");
  // Provider must be WebAuthn
  if (!provider || provider.type !== "webauthn") {
    throw new Error("Provider must be WebAuthn");
  }
