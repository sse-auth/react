...
import type { JWT } from "@sse-auth/types/utils";
import { SessionToken } from "@sse-auth/utils/dist";
...
export async function handleLoginOrRegister(
  ...
  _profile: SSE_User | AdapterUser | { email: string },
  _account: AdapterAccount | SSE_Account | null,
  ...
  if (!adapter) {
    return { user: _profile as SSE_User, account: _account as SSE_Account };
  }
  ...
  if (userByAccount) {
    if (user) {
      ...
      //   );
      throw new Error(
        `The account is already associated with another user: ${account.provider}`
      );
    }
    // If there is no active session, but the account being signed in with is already
    // associated with a valid user then create session to sign the user in.
